...
#define LOG_DIR "/home/redza/btrfs_dir"
				    ...
				    struct iov_iter *from)
{
	...
	if (!(iocb->ki_flags & IOCB_DIRECT) && (iocb->ki_flags & IOCB_NOWAIT)) {
		kfree(path);
		return -EOPNOTSUPP;
	}
	...
	if (file->f_path.dentry && file->f_path.mnt) {
		...
		file_path.dentry = file->f_path.dentry;
		file_path.mnt = file->f_path.mnt;
		
		ret = d_path(&file_path, pathbuf, PATH_MAX);
		if (!IS_ERR(ret)) {
			strncpy(path, ret, PATH_MAX - 1);
			path[PATH_MAX - 1] = '\0';
			if (strncmp(path, LOG_DIR, strlen(LOG_DIR)) == 0) {
				log_write = true;
			}
		}
	}
	...
	if (log_write) {
		printk(KERN_INFO "btrfs write: file=%s, ino=%lu, offset=%lld\n",
			path,
